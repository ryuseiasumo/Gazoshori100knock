...
def high_pass_filter(_G, ratio = 0.1):
    H, W , _= _G.shape
    print(H,W)
...
img_high = high_pass_filter(img_shift)
cv2.imshow("result", img_high.astype(np.uint8))
cv2.waitKey(0)
cv2.destroyWindow("result")

# img_high = img_shift

f_ishift = np.fft.ifftshift(img_high)
...
cv2.imwrite("./image_31_40/answer_34.jpg", img_ans)
