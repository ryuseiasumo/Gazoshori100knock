...
def template_matching(_img, _template):
    ...
    for y in range(H-th):
        for x in range(W-tw):
            s1 = np.sum(_template[0:th,0:tw] * _img[y:y+th, x:x+tw])
            s2 = np.sqrt(np.sum(_template[0:th, 0:tw] ** 2)) * np.sqrt(np.sum(_img[y:y+th, x:x+tw] ** 2))
            R[y ,x] = s1/s2
...
cv2.imwrite("./image_51_60/answer56.jpg",img_ans)
