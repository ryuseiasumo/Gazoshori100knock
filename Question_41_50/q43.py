...
def Canny_step3(_edge, HT = 50, LT = 20):
    H, W = _edge.shape
    edge = _edge.copy()
    for y in range(H):
        for x in range(W):
            dy1, dy2, dx1, dx2 = -1, 2, -1, 2
            ...
            if _edge[y,x] > LT & np.max(_edge[y+dy1:y+dy2,x+dx1:x+dx2]) > HT:
                edge[y, x] = 255
        print(edge)
...
edge = Canny_step3(edge, 50, 20)

# Save result
cv2.imwrite("./image_41_50/answer43.jpg", edge)
